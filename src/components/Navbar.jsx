...
const Navbar = ({setSelectedNav}) => {
  ...
  const toggleSelectedNav = (navOption) =>{
    if(navOption === 'all'){
      setSelectedNav('all');
    }
    else if(navOption === 'facebook'){
      setSelectedNav('facebook');
    }
  }
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isDropdownOpen && !event.target.closest('#profile-dropdown-menu') && event.target.id !== 'profile-dropdown-toggle') {
      ...
      <div className='flex flex-wrap items-center justify-between lg:gap-y-4 gap-y-6 gap-x-4 w-full'>
        ...
        <div id="collapseMenu" className={`max-lg:hidden lg:!block max-lg:before:fixed max-lg:before:bg-black max-lg:before:opacity-40 max-lg:before:inset-0 max-lg:before:z-50 ${isMenuOpen ? 'block' : 'hidden'}`}>
          ...
          <ul className='lg:flex lg:gap-x-10 max-lg:space-y-3 max-lg:fixed max-lg:bg-white max-lg:w-2/3 max-lg:min-w-[300px] max-lg:top-0 max-lg:left-0 max-lg:p-4 max-lg:h-full max-lg:shadow-md max-lg:overflow-auto z-50'>
            ...
            <li onClick={()=>{toggleSelectedNav('all'); console.log('clicked all notice', new Date())}}  className='max-lg:border-b max-lg:py-3 max-lg:px-3 relative lg:after:absolute lg:after:bg-black lg:after:w-full lg:after:h-[2px] lg:after:block lg:after:top-7 lg:after:transition-all lg:after:duration-300'>
              <a href='javascript:void(0)' className='text-black block text-[15px]' >All Notices</a>
            </li>
            <li onClick={()=>{toggleSelectedNav('facebook');console.log('clicked Facebook Notices', new Date())}} className='max-lg:border-b max-lg:py-3 max-lg:px-3 relative lg:hover:after:absolute lg:after:bg-black lg:after:w-0 lg:hover:after:w-full lg:hover:after:h-[2px] lg:after:block lg:after:top-7 lg:after:transition-all lg:after:duration-300'>
              <a href='javascript:void(0)' className='text-black block text-[15px]' >Facebook Notices</a>
            </li>
        ...
        <div className='flex items-center max-sm:ml-auto space-x-6'>
          <ul>
            <li id="profile-dropdown-toggle" className="relative px-1 after:absolute after:bg-black after:w-full after:h-[2px] after:block after:top-8 after:left-0 after:transition-all after:duration-300" onClick={handleDropdownToggle}>
              <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" className="cursor-pointer hover:fill-black" viewBox="0 0 512 512">
              ...
              <div id="profile-dropdown-menu" className={`bg-white ${isDropdownOpen ? 'block' : 'hidden'} z-20 shadow-lg py-6 px-6 rounded sm:min-w-[320px] max-sm:min-w-[250px] absolute right-0 top-10`}>
                ...
                <button type='button' className="bg-transparent border border-gray-300 hover:border-black rounded px-4 py-2 mt-4 text-sm text-black">LOGIN / SIGNUP</button>
